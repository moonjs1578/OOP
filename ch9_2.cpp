...
SmartPhone* Person:: newSmartPhone(){
	if(id % 2 == 1){
		GalaxyPhone *galaxy = new GalaxyPhone(this->name);
		return galaxy;
	}
	else{
		IPhone* iphone = new IPhone(this->name, "13");
		return iphone;
	}
}
...
class Student : virtual public Person /* TODO: Person 클래스 상속 받기 */ {
...
class Worker : virtual public Person /* TODO: Person 클래스 상속 받기 */ {
    string company;    // 회사명
    string position;   // 직급
...
class StudentWorker : public Student, public Worker /* TODO: [문제 1]Student와 Worker 클래스들을 다중 상속 받아라. */
{
    string career;      // 알바경력
...
void PersonManager::find() { // Menu item 9
    ...
    if (!found) {
            cout << "NOT found by operator ==" << std::endl;
        }
    delete p;
}
