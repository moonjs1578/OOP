...
bool Student::operator==(const Student& s){
	return department == s.department && year == s.year;
}
...
void Student::takeClass() {
	cout << getName() << "took several coures and got GPA " << GPA << endl;
}
...
Worker::Worker(const Worker& w) : Person(w), company(w.company), position(w.position){
	cout << "Worker::Worker(const Worker&):";
	printMembers(cout);
	cout << endl;
}
Worker::~Worker() {
...
bool Worker::operator==(const Worker& w){
	return (company == w.company && position == w.position);
}
...
StudentWorker::StudentWorker(const string &name, int id, double weight,
		bool married, const char *address, const string &department, double GPA,
		int year, const string &company, const string &position,
		const string &career, bool male) :
		Person(name, id, weight, married, address), // ✅ 가상 상속된 부모 직접 생성
		Student(name, id, weight, married, address, department, GPA, year), Worker(
				name, id, weight, married, address, company, position), career(
				career),   // ✅ StudentWorker 멤버 초기화
		male(male)        // ✅ StudentWorker 멤버 초기화
{
...
bool StudentWorker::operator==(const StudentWorker& a){
	return ((*(Student*)this == a) && (*(Worker*)this == a) && (male == a.male));
}
void StudentWorker::whatAreYouDoing() {
    cout << "########### StudentWorker::whatAreYouDoing() ##############\n";
    cout << getName() << " is studying as a " << getYear() << "-year student in "<< getDepartment() << endl;
    cout << getName() << " works in " << getCompany() << " as " << getPosition() << endl;
    cout << getName() << " took several courses and got GPA" << getGPA() << endl;
    cout << getName() << " is now enjoying his(her) vacation " << endl;
...
class MultiManager
{
	...
	Student students[studentCount] = { Student("s1", 1, 65.4, true,
			"Jongno-gu Seoul", "Physics", 3.8, 1), Student("s2", 2, 54.3, false,
			"Yeonje-gu Busan", "Electronics", 2.5, 4), };

	static const int workerCount = 2;
	Worker workers[workerCount] = { Worker("w1", 3, 33.3, false,
			"Kangnam-gu Seoul", "Samsung", "Director"), Worker("w2", 4, 44.4,
			true, "Dobong-gu Kwangju", "Hyundai", "Manager"), };

	static const int albaCount = 2;
	StudentWorker albas[albaCount] = { StudentWorker("a1", 5, 55.5, true,
			"Dong-gu Incheon", "Computer", 3.5, 2, "Hyundai", "Labor",
			"CU KangNam,Seven Eleven,GSStore Suwon", false), StudentWorker("a2",
			6, 66.6, false, "Sasang-gu Sejong", "History", 3.1, 1, "Kia", "CEO",
...
class Inheritance
{
    ...
    void student() {
    	...
    	s2.setDepartment (s1.getDepartment ()+"-Electronics"); s2. setGPA(s1.getGPA ()+1);
	...
	void studentWorker() {
		...
		sw2.set("a2", sw1.getId() + 1, sw1.getWeight() * 1.1, !sw1.getMarried(),
				sw1.getAddress());
